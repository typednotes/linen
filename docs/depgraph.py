...
DONE = {
    ...
    "Hale.Warp.Network.Wai.Handler.Warp.Counter",   # ported to Linen/Network/WebApp/Server/Counter.lean (namespace Network.WebApp.Server); verbatim, renamed package Warp -> Server per this project's Network.HTTP2.Server/Network.QUIC.Server naming convention
    "Hale.Warp.Network.Wai.Handler.Warp.Date",   # ported to Linen/Network/WebApp/Server/Date.lean (namespace Network.WebApp.Server): AutoUpdate-backed HTTP-date cache; import corrected from Hale.AutoUpdate to Linen.Control.AutoUpdate
    "Hale.Warp.Network.Wai.Handler.Warp.HashMap",   # ported to Linen/Network/WebApp/Server/HashMap.lean (namespace Network.WebApp.Server); verbatim
    "Hale.Warp.Network.Wai.Handler.Warp.Header",   # ported to Linen/Network/WebApp/Server/Header.lean (namespace Network.WebApp.Server); verbatim
    "Hale.Warp.Network.Wai.Handler.Warp.PackInt",   # ported to Linen/Network/WebApp/Server/PackInt.lean (namespace Network.WebApp.Server); String.mk -> String.ofList (deprecation fix), otherwise verbatim
    "Hale.Warp.Network.Wai.Handler.Warp.ReadInt",   # ported to Linen/Network/WebApp/Server/ReadInt.lean (namespace Network.WebApp.Server); verbatim
    "Hale.Warp.Network.Wai.Handler.Warp.Request",   # ported to Linen/Network/WebApp/Server/Request.lean (namespace Network.WebApp.Server): HTTP request-line/header parsing off a Socket.FFI.RecvBuffer, parseRequest builds a Network.WebApp.Request; imports Network.HTTP.Types.{Header,Method,URI,Version} individually (no aggregator file for that directory)
    "Hale.Warp.Network.Wai.Handler.Warp.Settings",   # ported to Linen/Network/WebApp/Server/Settings.lean (namespace Network.WebApp.Server): Settings/defaultSettings, settingsServerName rebranded "Linen/WebApp.Server"
    "Hale.Warp.Network.Wai.Handler.Warp.Response",   # ported to Linen/Network/WebApp/Server/Response.lean (namespace Network.WebApp.Server): status-line/header rendering, sendResponse (Blocking) and sendResponseEL (EventDispatcher/Green); String.mk -> String.ofList fix
    "Hale.Warp.Network.Wai.Handler.Warp.Run",   # ported to Linen/Network/WebApp/Server/Run.lean (namespace Network.WebApp.Server): runSettings/runSettingsEventLoop, acceptLoop/acceptLoopEL rewritten from partial self-recursion to `while true do`/`while keepGoing do` loops (no partial def, no fuel)
    "Hale.Warp.Network.Wai.Handler.Warp.Types",   # ported to Linen/Network/WebApp/Server/Types.lean (namespace Network.WebApp.Server); import corrected from Hale.Network/Hale.TimeManager to Linen.Network.Socket.Types/Linen.System.TimeManager
    "Hale.Warp.Network.Wai.Handler.Warp.Conduit",   # ported to Linen/Network/WebApp/Server/Conduit.lean (namespace Network.WebApp.Server): ISource (mkKnown/mkChunked/read); verbatim
    "Hale.Warp.Network.Wai.Handler.Warp.IO",   # ported to Linen/Network/WebApp/Server/IO.lean (namespace Network.WebApp.Server): connSendByteArray/connSendByteArrays; verbatim
    "Hale.Warp.Network.Wai.Handler.Warp.SendFile",   # ported to Linen/Network/WebApp/Server/SendFile.lean (namespace Network.WebApp.Server): sendFileWithConn/readSendFile; verbatim
    "Hale.Warp.Network.Wai.Handler.Warp.Internal",   # ported to Linen/Network/WebApp/Server/Internal.lean (namespace Network.WebApp.Server): pure re-export aggregator of Types/Settings/Date/Header/Counter/ReadInt/PackInt/IO/HashMap/Conduit/SendFile
    "Hale.Warp.Network.Wai.Handler.Warp.WithApplication",   # ported to Linen/Network/WebApp/Server/WithApplication.lean (namespace Network.WebApp.Server): withApplicationSettings/withApplication; bug fix over Hale source - now reads back the real OS-assigned port via Network.Socket.getSockName instead of always passing 0 to `action`
    "Hale.Warp.Network.Wai.Handler.Warp",   # ported to Linen/Network/WebApp/Server.lean (namespace Network.WebApp.Server): package aggregator + `run` convenience entry point; renamed from Network.Wai.Handler.Warp per the Server naming convention
    "Hale.Warp",   # aggregator: package-root re-export of the 17 Network.Wai.Handler.Warp.* modules above; covered by linen's root; no file
    "Hale.WarpQUIC.Network.Wai.Handler.WarpQUIC",   # ported to Linen/Network/WebApp/Server/QUIC.lean (namespace Network.WebApp.Server.QUIC): Settings/defaultSettings/toQUICConfig/toH3Settings/h3RequestToHeaders/handleConnection/runH3/runQUIC bridging Network.WebApp to Network.QUIC + Network.HTTP3; renamed from WarpQUIC per the Server naming convention (matching Network.QUIC.Server)
    "Hale.WarpQUIC",   # aggregator: package-root re-export; covered by linen's root; no file
    "Hale.WarpTLS.Network.Wai.Handler.WarpTLS",   # ported to Linen/Network/WebApp/Server/TLS.lean (namespace Network.WebApp.Server.TLS): OnInsecure/CertSettings/TLSSettings, tlsConnection/tlsAcceptLoop rewritten off `while` loops (already non-partial), runTLS; renamed from WarpTLS per the Server naming convention
    "Hale.WarpTLS.Network.Wai.Handler.WarpTLS.Internal",   # ported to Linen/Network/WebApp/Server/TLS/Internal.lean (namespace Network.WebApp.Server.TLS.Internal): trivial re-export stub, matching Hale's original (also empty)
    "Hale.WarpTLS",   # aggregator: package-root re-export; covered by linen's root; no file
    "Hale.WebSockets.Network.WebSockets.Types",   # ported to Linen/Network/WebSockets/Types.lean (namespace Network.WebSockets, no renaming needed): Opcode/CloseCode/ConnectionState/ConnectionOptions/RequestHead/Connection/PendingConnection/ServerApp + 16 opcode roundtrip theorems; verbatim
    "Hale.WebSockets.Network.WebSockets.Frame",   # ported to Linen/Network/WebSockets/Frame.lean (namespace Network.WebSockets): Frame + applyMask/Frame.encode/Frame.decode (RFC 6455 SS5 framing); verbatim
    "Hale.WebSockets.Network.WebSockets.Connection",   # ported to Linen/Network/WebSockets/Connection.lean (namespace Network.WebSockets): mkConnection (send/recv -> full Connection with framing, masking, control-frame handling); verbatim
    "Hale.WebSockets.Network.WebSockets.Handshake",   # ported to Linen/Network/WebSockets/Handshake.lean (namespace Network.WebSockets): webSocketGUID/computeAcceptKey/isValidHandshake/buildHandshakeResponse; Data.ByteString.Base64.encode corrected to Linen's Data.Base64.encode; honest non-functional SHA-1 placeholder (with TODO) preserved as-is, out of scope to implement
    "Hale.WebSockets",   # ported to Linen/Network/WebSockets.lean: package aggregator re-exporting Types/Frame/Handshake/Connection
    "Hale.WaiWebSockets.Network.Wai.Handler.WebSockets",   # ported to Linen/Network/WebApp/Server/WebSockets.lean (namespace Network.WebApp.Server.WebSockets): isWebSocketsReq/websocketsApp/websocketsOr bridging Network.WebApp requests to Network.WebSockets connections via responseRaw; renamed from Network.Wai.Handler.WebSockets per the Server naming convention
    "Hale.WaiWebSockets",   # aggregator: package-root re-export; covered by linen's root; no file
    "Hale.Word8.Data.Word8",   # ported to Linen/Data/Word8.lean (namespace Data.Word8): UInt8 ASCII classification predicates/case conversion/byte constants, with native_decide idempotency/coherence proofs over all 256 UInt8 values; verbatim
    "Hale.Word8",   # aggregator: package-root re-export; covered by linen's root; no file
    "Hale",   # the root Hale package aggregator (import Hale.<Package> for every one of Hale's ~90 sub-packages); no dedicated Linen file - Linen.lean already plays this role for the entire library
}
