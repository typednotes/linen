/-
  Examples.TLS — `Network.TLS.Types` and `Network.TLS.Context` end-to-end.

  Exercises a full TLS handshake over a real loopback TCP connection, using a
  locally-generated, 100-year-validity self-signed certificate for
  `CN=localhost` (embedded below as string literals — a throwaway demo
  fixture, not a production secret; the same fixture backs
  `Tests/Linen/Network/TLS/ContextTest.lean`).

  * the server side uses `createContext`/`setAlpn`/`acceptSocket`;
  * the client side uses `createClientContextWithCA`, trusting the
    self-signed certificate directly as its own CA, so the handshake
    succeeds fully offline without touching the system trust store, and
    `connectSocket` for the SNI + hostname-verified handshake;
  * `getVersion` checks the negotiated protocol is TLS 1.2 or 1.3;
  * `getAlpn` is still `none` on both ends — `setAlpn` only registers the
    server's selection callback (`h2` then `http/1.1`), and nothing in the
    current client API advertises a protocol list for it to select from.

  Args: (none) -- runs the round trip and exits non-zero on any mismatch
-/
import Linen.Network.TLS.Types
import Linen.Network.TLS.Context
import Linen.Network.Socket.Blocking

open Network.Socket
open Network.Socket.Blocking
open Network.TLS

namespace Examples.TLS

private def testCertPem : String :=
"-----BEGIN CERTIFICATE-----

MbICLhJY3hrA4sWYTOqj
-----END CERTIFICATE-----
"

private def testKeyPem : String :=
"-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
"

def demoHandshake : IO Bool := do
  IO.println "── TLS 1.2/1.3 handshake over loopback, self-signed CN=localhost ──"
  let (certHandle, certPath) ← IO.FS.createTempFile
  certHandle.putStr testCertPem
  certHandle.flush
  let (keyHandle, keyPath) ← IO.FS.createTempFile
  keyHandle.putStr testKeyPem
  keyHandle.flush

  let serverCtx ← createContext certPath.toString keyPath.toString
  setAlpn serverCtx
  -- The self-signed cert is its own issuer, so trusting it directly as the
  -- CA lets the client verify the server without touching the system store.
  let clientCtx ← createClientContextWithCA certPath.toString

  let server ← listenTCP "127.0.0.1" 0
  let addr ← getSockName server
  IO.println s!"  server listening on 127.0.0.1:{addr.port}"

  let serverTask ← IO.asTask (prio := .dedicated) do
    let (conn, _peer) ← Blocking.accept server
    let session ← acceptSocket serverCtx conn.raw
    let request ← read session 4096
    write session (request ++ "!".toUTF8)
    close session
    let _ ← Network.Socket.close conn
    pure request

  let clientSock ← socket .inet .stream
  let conn ← Blocking.connect clientSock { host := "127.0.0.1", port := addr.port }
  let session ← connectSocket clientCtx conn.raw "localhost"
  write session "ping".toUTF8
  let reply ← read session 4096
  let version ← getVersion session
  let alpn ← getAlpn session
  close session
  let _ ← Network.Socket.close conn
  let _ ← Network.Socket.close server

  let request ←
    match serverTask.get with
    | .ok bytes => pure bytes
    | .error e => throw e

  IO.FS.removeFile certPath
  IO.FS.removeFile keyPath

  IO.println s!"  server received {String.fromUTF8! request}, client received {String.fromUTF8! reply}"
  IO.println s!"  negotiated version: {version}, ALPN: {alpn}"

  let versionOk := version == "TLSv1.2" || version == "TLSv1.3"
  pure (request == "ping".toUTF8 && reply == "ping!".toUTF8 && versionOk && alpn == none)

def run (_args : List String) : IO Unit := do
  if ← demoHandshake then
    IO.println "\ntls demo done · all checks passed"
  else
    throw (IO.userError "tls demo done · some checks failed")

end Examples.TLS
